function CBIG_ABCD_KRR_4covariates(sites, innerFolds, feature_path, featurebase, outdir, ...
    subtxt, subcsv, predvar, covtxt, ymat, covmat)
% CBIG_ABCD_KRR_4covariates(sites, innerFolds, feature_path, featurebase, outdir,
%     subtxt, subcsv, predvar, covtxt, ymat, covmat)
%
% Prepare input parameters and run the single-kernel ridge regression (KRR)
% leave-p-out workflow with 4 fixed covariates (age, age^2, sex, site/motion).
%
% Input:
%     - sites: (scalar)
...
%
%     - covtxt: (string)
%           Full path to a text file listing 4 covariate names.
%
%     - ymat: (string)
...
%
% Example:
%     CBIG_ABCD_KRR_4covariates(3, 10, '/path/FC_final_Y0', 'FC_final_Y0', '/path/out', ...
...
if ~exist(fullfile(outdir,cov_mat))
    ...
    cov_types = {'continuous','continuous','categorical','continuous'}; % define covariate types
