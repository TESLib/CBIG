...
set VERSION = '$Id: CBIG_LBC_FCmetrics_wrapper_Nmin.csh, v 1.0 2025/02/05 $'
...
if ( $matlab_runtime == 0 ) then
    ...
    setenv LD_LIBRARY_PATH ${MATLAB}/runtime/glnxa64:${MATLAB}/bin/glnxa64:\
${MATLAB}/sys/os/glnxa64:${MATLAB}/sys/opengl/lib/glnxa64:${LD_LIBRARY_PATH}
...
if (! -e "$output_dir/$subject/logs") then
    mkdir -p "$output_dir/$subject/logs"
endif
set LF = $output_dir/$subject/logs/CBIG_LBC_FCmetrics_wrapper_Nmin.log
set all_LF = $output_dir/$subject/logs/CBIG_LBC_FCmetrics_wrapper_Nmin.log
# Ensure the log file exists before accessing it
if (! -e "$LF") then
    touch "$LF"
endif
...
echo "[CMD]: CBIG_LBC_FCmetrics_wrapper_Nmin.csh $cmdline"   >>$LF
...
"from Schaefer2018 400Parcels with Yeo17Networks. Please check!"  |& tee -a $LF
    ...
    if ( "$parcellation_type" == 'Yan' ) then
        ...
        set lh_cortical_ROIs_file = "$CBIG_CODE_DIR/stable_projects/"\
"brain_parcellation/Yan2023_homotopic/parcellations/FreeSurfer/fsaverage6/label"
        set rh_cortical_ROIs_file = "$CBIG_CODE_DIR/stable_projects/"\
"brain_parcellation/Yan2023_homotopic/parcellations/FreeSurfer/fsaverage6/label"
...
set FCmetrics_folder = "$output_dir/$subject/FC_metrics"
...
if ($Pearson_r == 1) then
    ...
    set flag_file = "${FCmetrics_folder}/${output_prefix}.flag"

    # If the flag file exists, skip MATLAB computation and exit immediately.

    if (-e $flag_file) then
        echo "Flag file $flag_file exists. Skipping FC metrics computation for $output_prefix." |& tee -a $LF
        exit 0
    endif

    # Prepare MATLAB arguments
    ...
    set matlab_args = "${matlab_args} '$discard_frames_list' 'Pearson_r' '$FCmetrics_folder' '$output_prefix'"


    # Run MATLAB or MATLAB runtime
    if ($matlab_runtime == 0) then
        set cmd = ( $MATLAB -nodesktop -nodisplay -nosplash -r '"' "addpath(genpath('${root_dir}'));")
        set cmd = ( $cmd CBIG_LBC_preproc_FCmetrics_Nmin $matlab_args; exit; '"' )
    else
        set cmd = ( ${matlab_runtime_util}/CBIG_LBC_preproc_FCmetrics_Nmin $matlab_args )
    endif
...
#CBIG_LBC_FCmetrics_wrapper_Nmin.csh >> $LF
 #   popd
#endif
...
if ( ( "$parcellation_name" == "" && "$lh_cortical_ROIs_file" != "" ) || \
     ( "$parcellation_name" == "" && "$rh_cortical_ROIs_file" != "" ) ) then
...
if ( ( "$network" != "" ) && ( "$network" != "Yeo7" ) && ( "$network" != "Yeo17" ) && \
     ( "$network" != "Kong17" ) ) then
    echo "ERROR: parcellation type not supported, should be 'Yeo7', 'Yeo17' or 'Kong17'"
    exit 1;
endif

if ( ( "$parcellation_type" != "" ) && ( "$parcellation_type" != "Yan" ) && \
     ( "$parcellation_type" != "Schaefer" ) ) then
...
NAME:
    CBIG_LBC_FCmetrics_wrapper_Nmin.csh

DESCRIPTION:
    Compute FC (functional connectivity) metrics for cortical and 19 subcortical ROIs.
    Parcellation used in this function can be set to Schaefer2018 or Yan2023.
    Please refer to -p_type, -network, and -res.
...
REQUIRED ARGUMENTS:
    ...
    -nframes:
     number of frames that extracted from the timeseries to compute FC
...
OPTIONAL ARGUMENTS:
    ...
    -res resolution 
     Schaefer2018 and Yan2023 parcellation have different resolutions. Input should be
     hundreds between 100 to 1000 from 100 to 1000.
     Default resolution is 400.
     -res only works with Schaefer2018 and Yan2023 parcellation. This argument will be
     ignored if custom parcellation files are used.

    -network network
     Schaefer2018 and Yan2023 parcellation have different versions, with parcels matched
     to different versions of networks.
     Input should be 'Yeo7', 'Yeo17', or 'Kong17' (case sensitive). Default network is 'Kong17'.
     -network only works with Schaefer2018 and Yan2023 parcellation. This argument will be
     ignored if custom parcellation files are used.
    ...
    -p_name parcellation_name
     ...
     Please note that if parcellation_name is passed in, both lh_cortical_ROIs_file
     and rh_cortical_ROIs_file should also
     be passed in.
    ...
    -nocleanup :
     do not remove intermediate result. For example, if -Pearson_r is used, intermediate
     files are lh2lh, lh2rh, rh2rh, lh2subcortical, rh2subcortical,
     and subcortical2subcortical correlation files.

    -matlab_runtime :
     running MATLAB code using MATLAB Runtime instead of MATLAB.

EXAMPLE with default setting:
    $CBIG_CODE_DIR/stable_projects/preprocessing/CBIG_fMRI_Preproc2016/CBIG_LBC_FCmetrics_wrapper_Nmin.csh
    -s Sub0001_Ses1 -d ~/storage/fMRI_preprocess -bld "002 003"
    -BOLD_stem _rest_skip4_stc_mc_residc_interp_FDRMS0.2_DVARS50_bp_0.009_0.08
